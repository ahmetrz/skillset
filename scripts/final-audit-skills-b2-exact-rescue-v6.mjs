...
const PART=Number(process.env.AUDIT_PARTITION||0),PARTS=Math.max(1,Number(process.env.AUDIT_PARTITIONS||4));
...
const assigned=unresolved.filter((_,i)=>i%PARTS===PART);
...
const base='skills-b2-exact-rescue-p'+String(PART).padStart(2,'0')+'-of-'+String(PARTS).padStart(2,'0');
