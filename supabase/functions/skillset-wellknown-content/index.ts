...
async function loadHostIndex(host:string):Promise<Map<string,IndexHit>>{
 ...
 for(const c of candidates){
  try{
   ...
   if(j?.$schema===V2_SCHEMA){for(const e of j.skills){if(typeof e?.name!=='string'||e?.type!=='skill-md'||typeof e?.url!=='string'||typeof e?.digest!=='string')continue;try{out.set(e.name,{kind:'v2',url:new URL(e.url,c.url).toString(),digest:e.digest});}catch{}}}
   else if(j?.$schema===undefined){for(const e of j.skills){if(typeof e?.name!=='string'||!Array.isArray(e?.files)||!e.files.some((f:any)=>typeof f==='string'&&f.toLowerCase()==='skill.md'))continue;out.set(e.name,{kind:'v1',base:`https://${host}`,path:c.path});}}
  }catch{}
 }
 return out;
}
Deno.serve(async(req)=>{
 ...
 const limit=Math.max(1,Math.min(Number(body.limit??25),25));
 ...
 const processRow=async(row:any)=>{
  try{
   ...
   if(hit?.kind==='v2'){
    const r=await fetch(hit.url,{headers:{'user-agent':'skillset-corpus/1.0'},signal:AbortSignal.timeout(15000)});if(r.ok){const bytes=new Uint8Array(await r.arrayBuffer());const digest=await sha256Bytes(bytes);const expected=hit.digest.replace(/^sha256:/,'').toLowerCase();if(digest!==expected)throw new Error('wellknown_digest_mismatch');const candidate=new TextDecoder().decode(bytes);if(looksLikeSkillMd(candidate)){text=candidate;sourceUrl=hit.url;digestVerified++;}}
   }else if(hit?.kind==='v1'){
    const u=`${hit.base}/${hit.path}/${encodeURIComponent(skill)}/SKILL.md`;const r=await fetch(u,{headers:{'user-agent':'skillset-corpus/1.0'},signal:AbortSignal.timeout(15000)});if(r.ok){const candidate=await r.text();if(looksLikeSkillMd(candidate)){text=candidate;sourceUrl=u;}}
   }
   if(text===null){const direct=[`https://${host}/.well-known/agent-skills/${encodeURIComponent(skill)}/SKILL.md`,`https://${host}/.well-known/skills/${encodeURIComponent(skill)}/SKILL.md`];for(const u of direct){const r=await fetch(u,{headers:{'user-agent':'skillset-corpus/1.0'},signal:AbortSignal.timeout(15000)});if(!r.ok)continue;const candidate=await r.text();if(looksLikeSkillMd(candidate)){text=candidate;sourceUrl=u;break;}}}
 ...
 const concurrency=5;for(let i=0;i<(rows??[]).length;i+=concurrency)await Promise.all((rows??[]).slice(i,i+concurrency).map(processRow));
