...
Deno.serve(async(req)=>{
 ...
 const source=String(row.source),owner=String(row.owner),repo=String(row.repo); let branch:string|null=null,files=0,newSkills=0;
 try{
  const headers={'accept':'application/vnd.github+json','user-agent':'skillset-corpus/1.0'};
  const meta=await fetch(`https://api.github.com/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}`,{headers,signal:AbortSignal.timeout(15000)});
  if(!meta.ok)throw new Error(`repo_http_${meta.status}`); const mj=await meta.json(); branch=String(mj.default_branch||'main');
  const tree=await fetch(`https://api.github.com/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/git/trees/${encodeURIComponent(branch)}?recursive=1`,{headers,signal:AbortSignal.timeout(20000)});
  if(!tree.ok)throw new Error(`tree_http_${tree.status}`); const tj=await tree.json();
  ...
  for(let start=0;start<paths.length;start+=concurrency){await Promise.all(paths.slice(start,start+concurrency).map(async(path:string)=>{
    ...
    const {data:inserted,error:ue}=await db.rpc('skillset_upsert_repo_discovered_skill',{p_source:source,p_skill_name:skillName,p_repo_path:parent,p_owner:owner,p_repo:repo,p_source_url:`https://github.com/${owner}/${repo}/blob/${branch}/${path}`,p_sha:hash,p_raw_bytes:enc.encode(text).byteLength,p_object_key:objectKey,p_gzip_bytes:gz.byteLength}); if(!ue&&inserted===true)newSkills++;
  }));}
  await db.rpc('skillset_finish_repository',{p_source:source,p_branch:branch,p_files:files,p_new:newSkills,p_error:null});
  return new Response(JSON.stringify({ok:true,source,branch,files,newSkills}),{headers:{'content-type':'application/json'}});
 }catch(e){const m=e instanceof Error?e.message:String(e);await db.rpc('skillset_finish_repository',{p_source:source,p_branch:branch,p_files:files,p_new:newSkills,p_error:m});return new Response(JSON.stringify({ok:false,source,error:m}),{status:500,headers:{'content-type':'application/json'}});}
});